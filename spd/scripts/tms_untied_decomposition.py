...
class Model(nn.Module):
    def __init__(
        ...
        self.C = nn.Parameter(
            torch.empty((config.n_instances, config.n_hidden, config.k), device=device)
        )
        self.D = nn.Parameter(
            torch.empty((config.n_instances, config.k, config.n_features), device=device)
        )
    ...
    def forward(
        ...
        h_1 = torch.einsum("...ih,ihk->...ik", hidden, self.C)
        pre_relu = torch.einsum("...ik,ikf->...if", h_1, self.D)
    ...
    with trange(steps) as t:
        for step in t:
            ...
            for feature_idx in range(out.shape[-1]):
                ...
                grad_h_0 = torch.einsum("...ih,ikh->...ik", grad_hidden.detach(), model.B)
                grad_h_1 = torch.einsum("...if,ikf->...ik", grad_pre_relu.detach(), model.D)
                sparsity_inner = (
                    # (grad_h_0.detach() * h_0) ** 2 + (grad_h_1.detach() * h_1) ** 2 + 1e-16
                    # (grad_h_1.detach()) ** 2 + 1e-16
            ...
            loss.backward()
            opt.step()
            # Force the A and C matrices to have norm 1 in the features dimension
            model.A.data = model.A.data / model.A.data.norm(p=2, dim=-2, keepdim=True)
            # Also make B have norm 1 in the hidden dimension
            model.B.data = model.B.data / model.B.data.norm(p=2, dim=-1, keepdim=True)
            model.D.data = model.D.data / model.D.data.norm(p=2, dim=-1, keepdim=True)
            W2 = einops.rearrange(model.C.detach() @ model.D.detach(), "i h f -> i f h")
            C = einops.rearrange(model.C.detach(), "i h k -> i k h")
            if step % print_freq == print_freq - 1 or step == 0:
                # tqdm.write(f"Reconstruction loss: {recon_loss.item()}")
                # tqdm.write(f"Sparsity loss: {sparsity_loss.item()}")
                tqdm.write(f"W2 after {step + 1} steps (before gradient update)")
                plot_intro_diagram(model, weight=W2)
                tqdm.write(f"C after {step + 1} steps (before gradient update)")
                plot_intro_diagram(model, weight=C)
                tqdm.write(f"W2 after {step + 1} steps (before gradient update) abs normed")
                # Take the absolute value and normalize the hidden dimension
                W2_abs_normed = torch.abs(W2) / torch.abs(W2).norm(p=2, dim=-1, keepdim=True)
                C_abs_normed = torch.abs(C) / torch.abs(C).norm(p=2, dim=-1, keepdim=True)
                prev_n_instances = model.config.n_instances
                model.config.n_instances = 4
                plot_intro_diagram(model, weight=torch.abs(W2_abs_normed)[:4])
                tqdm.write(f"C after {step + 1} steps (before gradient update) abs")
                plot_intro_diagram(model, weight=torch.abs(C_abs_normed)[:4])
...
if __name__ == "__main__":
    ...
    print("Plot of W2 after training")
    W2 = einops.rearrange(model.C.detach() @ model.D.detach(), "i h f -> i f h")
    C = einops.rearrange(model.C.detach(), "i h k -> i k h")
    # Take the absolute value and normalize the hidden dimension
    W2_abs_normed = torch.abs(W2) / torch.abs(W2).norm(p=2, dim=-1, keepdim=True)
    C_abs_normed = torch.abs(C) / torch.abs(C).norm(p=2, dim=-1, keepdim=True)
    plot_intro_diagram(model, weight=W2_abs_normed[4:10])
    # plot_intro_diagram(model, weight=torch.abs(model.C.detach() @ model.D.detach()).T[4:10])
    print("Plot of C after training")
    plot_intro_diagram(model, weight=C_abs_normed[4:10])
