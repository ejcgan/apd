...
class _HookFunctionProtocol(Protocol):
    ...
    def __call__(self, tensor: torch.Tensor, *, hook: "HookPoint") -> Any | None: ...


HookFunction = _HookFunctionProtocol  # Callable[..., _HookFunctionProtocol]

DeviceType = torch.device | None
    ...
    def remove_hooks(
        ...
        def _remove_hooks(handles: list[LensHandle]) -> list[LensHandle]:
            ...
            for handle in handles:
                if (
                    including_permanent
                    or (not handle.is_permanent)
                    and (level is None or handle.context_level == level)
                ):
                    handle.hook.remove()
                else:
...
class HookedRootModule(nn.Module):
    ...
    def _enable_hook_with_name(
        self, name: str, hook: Callable[..., Any], dir: Literal["fwd", "bwd"]
    ) -> None:
        """Takes a key for the mod_dict and enables the related hook for that module.
    ...
    def hooks(
        ...
        """Context manager for adding temporary hooks to the model.
